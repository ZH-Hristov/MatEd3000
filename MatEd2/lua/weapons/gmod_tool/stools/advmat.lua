...
TOOL.ClientConVar["setforsamemodel"] = "0"
TOOL.ClientConVar["usessbump"] = "0"
...
function TOOL:LeftClick(trace)
	...
	local usessbump = tonumber(self:GetClientInfo("usessbump"))
	...
	local ignorez = tonumber(self:GetClientInfo("ignorez"))
	
	local setforsamemodel = tobool(self:GetClientInfo("setforsamemodel"))
	...
	if setforsamemodel then 
		local cls = toSet:GetClass()
		local mdl = toSet:GetModel()
		if cls == "prop_effect" then
			mdl = toSet:GetChildren()[1]:GetModel()
		end
		timer.Simple(1, function()
			local matToSet = toSet:GetMaterial()
			local toSetEnts = ents.FindByClass(cls)
			for _, v in pairs(toSetEnts) do
				if v:GetClass() == cls then
					if v:GetClass() == "prop_effect" then
						local effect = v:GetChildren()[1]
						if effect:GetModel() == mdl then
							effect:SetMaterial(matToSet)
						end
					else
						if v:GetModel() == mdl then
							v:SetMaterial(matToSet)
						end
					end
				end
			end
...
do
	...
	function TOOL.BuildCPanel(CPanel)
		...
		CPanel:CheckBox("#tool.advmat.setforsamemodel", "advmat_setforsamemodel")
		CPanel:CheckBox("#tool.advmat.ignorez", "advmat_ignorez")
		...
		CPanel:CheckBox("#tool.advmat.usessbump", "advmat_usessbump")
		CPanel:ControlHelp("If this box is checked, your bumpmap will be marked as a self-shadowing bumpmap. WORKS ONLY WITH MATERIALS THAT SUPPORT IT, THESE USUALLY HAVE SSBUMP IN THEIR NAME!")
...
if (CLIENT) then
	...
	language.Add("tool.advmat.usessbump", "Enable Self-Shadowing Bump?")
	...
	language.Add("tool.advmat.setforsamemodel", "Set material for all entities with the same class and model?")
	language.Add("tool.advmat.ignorez", "Should this model ignore the Z-axis? (Render through everything)")
end